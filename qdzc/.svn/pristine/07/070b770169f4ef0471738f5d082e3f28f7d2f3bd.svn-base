...
<title>Insert title here</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {               	
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'040701','040702','040703'"></pageADMOperAuth.tld:pageADMOperAuth>
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	                ...
	                { display: '支局名称', name: 'areaName', width: 100, minWidth: 150 },
	                { display: '所属区县', name: 'areaNamepr', width: 100, minWidth: 60}
	                ],
	                dataAction: 'server',
	                pageSize:30,
	                where : f_getWhere(),
	                rownumbers:true, 
	                checkbox:true,
	                url: 'getsubStationQueryActionII',
	       ...
	       function f_closeAddWindow(item, dialog)
	   		...
	   		function f_add()
	        {
	   			...
	   			$.ligerDialog.open({
		        	   ...
		        	   height: 400, 
		        	   width: 700, 
		        	   url: 'sysManage/subStationAdd.jsp', 
		        	   showMax: true, 
		        	   isResize: true, 
		        	   slide: false,
	        ...
	        {	
	            var row = manager.getCheckedRows();
	           	...
	           	if (row.length == 0) { 
	       			...
	       			var actionURL = 'subStationModifyQueryAction?areaId='+row[0].areaId;
	       			
	       			$.ligerDialog.open({ title: '修改支/分局信息',height: 400, width: 700, url: actionURL, showMax: true,  isResize: true, slide : false,
	 	        	   buttons: [{ text: '关闭窗口', onclick: f_closeAddWindow }]}); 
