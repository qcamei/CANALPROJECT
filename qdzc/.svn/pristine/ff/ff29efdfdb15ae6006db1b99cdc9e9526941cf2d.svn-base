    ...
    <script type="text/javascript">
           $(function() {
        	  ...
        	  var v = $("form").validate({
       		...
       		var isPay = decodeURI("${param.isPay}");
       		var auditState = decodeURI("${param.auditState}");
       		var remark = decodeURI("${param.remark}");
       		$("#inId").val(inId);
       		$("#canalId").val(canalId);
       		$("#canalName").val(canalName);
       		
       	   if(isPay == "是"){
          	 $("input[name=isPay][value='是']").attr("checked","checked");
           }else{
   			 $("input[name=isPay][value='否']").attr("checked","checked");       	 
           }
    ...
    <form name="form1" method="post" action="" id="form1">
        ...
        <table cellpadding="0" cellspacing="0" class="l-table-edit" width="500px">
        	...
        	<tr>
             ...
             <tr>
                <td align="right" class="l-table-edit-td"><font color="red">*</font>是否收款(保证金):</td>
                <td align="left" class="l-table-edit-td"> 
                
                 	<input  type="radio" name="isPay" value="是" checked="checked" /><label for="isUseable">是</label> 
                    <input  type="radio" name="isPay" value="否" /><label for="isUseable">否</label>
                
                </td>
            </tr>
            
            <tr>
                <td align="right" class="l-table-edit-td"><font color="#ff0000">*</font>审核状态:</td>
                <td align="left" class="l-table-edit-td">
                	<input name="auditState" type="text" id="auditState" ltype="text" validate="{required:true}" value="通过" />
                </td>                
            </tr>
...
<script type="text/javascript">
  function ajaxUpdate(){//提交表单内容
			 ...
			 params = encodeURIComponent(encodeURIComponent(params)).replace(/%253D/g,'=').replace(/%2526/g,'&');
		      ...
		      $.ajax({   //把表单内容插入数据库
		              url :'financeModifyAction',  //后台处理程序   
		              type:'post',    //数据发送方式   
