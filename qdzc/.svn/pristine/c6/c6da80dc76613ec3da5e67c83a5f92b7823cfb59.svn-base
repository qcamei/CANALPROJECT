    ...
    <form name="form1" method="post" action="" id="form1">
        ...
        <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
        	<tr>
                ...
                <tr>
                <td align="right" class="l-table-edit-td"><font color="red">*</font>稽核厅名称:</td>
                <td align="left" class="l-table-edit-td"> 
                <input  ltype="text"  name="checkName" id="checkName" type="text" ltype="text" validate="{required:true}" />
                </td>
            </tr>
             <tr>
                <td align="right" class="l-table-edit-td"><font color="red">*</font>稽核厅编码:</td>
                <td align="left" class="l-table-edit-td"> 
                <input  ltype="text"  name="checkNumber" id="checkNumber" type="text" <c:if test="${request.checkNumber!='no' }"> value="${request.checkNumber }" readonly="readonly"</c:if>  ltype="text" validate="{required:true}"  />
                </td>
            </tr>
              <tr>
                <td align="right" class="l-table-edit-td"><font color="red">*</font> 资金稽核系统登录账号:</td>
                <td align="left" class="l-table-edit-td"> 
                <input  ltype="text"  name="moneyLogin" id="moneyLogin" type="text"  ltype="text" validate="{required:true}"  />
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
  function ajaxUpdate(){//提交表单内容
			 ...
			 params = encodeURIComponent(encodeURIComponent(params)).replace(/%253D/g,'=').replace(/%2526/g,'&');
		      ...
		      $.ajax({   //把表单内容插入数据库
		              url :'moneyCheckAddAction',  //后台处理程序   
		              type:'post',    //数据发送方式   
   ...
   function submit_Result(result){ //回传函数实体，参数为XMLhttpRequest.responseText   
  		...
  		if(result=='success') {
       		 window.parent.parent.$("#maingrid4").ligerGrid().loadData(true);		
	   			 ...
	   			 var parentWindow_body =  $(window.parent.parent.document).find("body");if(! parentWindow_body.find(".dealWithMask").length){parentWindow_body.append("<input type=text class=dealWithMask style=height:0;border:none/>");}parentWindow_body.find(".dealWithMask").focus();
