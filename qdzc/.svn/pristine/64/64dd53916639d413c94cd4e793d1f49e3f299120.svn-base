...
public class ShouyeListAction extends ActionSupport {
	...
	private String getResult() {
		...
		try {
			...
			sql = "SELECT @rownum :=@rownum + 1 AS rownum,t.* FROM(SELECT @rownum := 0) r,(select a.* from "
					+ "(SELECT p.*,q.in_id, q.current_step,q.canal_id,q.canal_name FROM "
					+ "(SELECT c.group_id,d.* FROM data_auth c right join "
					+ "( SELECT a.step_no,a.step_val cur_step, b.step_val next_step FROM qdzc.step_info a LEFT JOIN qdzc.step_info b ON a.step_no = b.pre_step_no WHERE b.step_val IS NOT NULL ) d"
					+ " on c.step_no = d.step_no AND c.group_id = '"
					+ groupId
					+ "' ) p LEFT JOIN canal_step_state q ON p.step_no = q.current_step where q.current_step<>'11' and q.dept_id in ("
					+ dataAuthId + ") ";
			...
			sql += " ) a left join qdzc.canal_infomation b on a.in_id=b.in_id where 1=1 ";
