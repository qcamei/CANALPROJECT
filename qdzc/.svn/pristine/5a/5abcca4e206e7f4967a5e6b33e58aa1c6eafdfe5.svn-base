...
public class OrganizationTree  extends Component{
	...
	private String getOrganization(){
		...
		if(deptId.length() > 1 && deptId.substring(3, 4).equals("1")){
			//1011001第4位的1表示企信部用户，完整树要显示
			deptId = deptId.substring(0, 3);
			resultStr.append("tree.add(new WebFXLoadTreeItem(\"市电信分公司\", \"organizationQueryAction?deptId="+deptId+"\",action=\"javascript:changeList('"+deptId.substring(0, 3)+":市电信分公司')\"));");
