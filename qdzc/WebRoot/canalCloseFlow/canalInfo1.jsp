...
<title>渠道信息列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'070101', '070102'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	                columns: [
	                    	...
	                    	{ display: '渠道ID', name: 'canalId', width: 70, minWidth: 60 },
...
function f_close(){
		var row = manager.getSelectedRow();		
   		...
   		if('正常'===row.canalState){   		
                $.ligerDialog.open({ 
                        	title: '申请关闭', 
                        	width: 500, 
                        	height: 400, 
                        	url: 'canalCloseReason.jsp?inId='+row.inId,
...
function f_detail(){
	...
	var m = $.ligerDialog.open({ 
  	  ...
  	  height: 400, 
  	  width: 700, 
           url:'detailQuery.jsp?inId='+row.inId,
  	   showMax: true, 
  	   isResize: true,      	 
  	   buttons: [{ text: '关闭窗口', onclick: f_closeAddWindow }]
    }); 
