...
<title>渠道关闭信息首列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'070201','070205'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	        	 columns: [
                	...
                	{ display: '下一步骤', name: 'nextStepVal', width: 70, minWidth: 60 },	    					
	    					...
	    					rownumbers:false, 
	    					checkbox:false,
	    		            url: 'canalCloseQueryFGSAction?type=fgs',
	...
	if(ints2!=1){
		...
		if('通过'==row.auditState){
            var m= $.ligerDialog.open({ 
                    	...
                    	width: 400, 
                    	height: 700, 
                    	url: 'canalCloseFlow/canalCloseReasonFGS.jsp?inId='+row.inId+'&closeId='+row.closeId+'&nextStepVal='+row.nextStepVal,                    			
                    	 showMax: true, 
                    	 isResize: true, 
