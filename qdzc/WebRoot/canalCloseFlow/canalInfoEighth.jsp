...
<title>渠道关闭信息首列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'070701','070702'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	       ...
	       /*  function get_curDay() {
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	        	 columns: [
	    					...
	    					rownumbers:false, 
	    					checkbox:false,
	    		            url: 'canalCloseQueryZJJHAction?type=zjjh',
	...
	if(ints2!=7){
		...
		if('通过'==row.auditState){
            var m= $.ligerDialog.open({ 
                    	...
                    	width: 400, 
                    	height: 700, 
                    	url: 'canalCloseFlow/canalCloseReasonZJJH.jsp?inId='+row.inId+'&closeId='+row.closeId+'&nextStepVal='+row.nextStepVal,                    			
                    	 showMax: true, 
                    	 isResize: true, 
