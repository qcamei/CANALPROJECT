...
<title>渠道关闭信息首列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'071101','071102'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	        	 columns: [
	                    	...
	                    	{ display: '当前欠款', name: 'money', width: 70, minWidth: 50 },	                   	
	                    	{ display: '当前步骤', name: 'currentStepVal', width: 70, minWidth: 60 },
	                    	{ display: '当前状态', name: 'auditState', width: 70, minWidth: 60 },
	    					...
	    					rownumbers:false, 
	    					checkbox:false,
	    		            url: 'canalCloseQueryCWSHAction?type=cwsh',
	...
	if(ints2!=11){
		...
		if('通过'==row.auditState){
            ...
            var m= $.ligerDialog.open({ 
                    	...
                    	width: 400, 
                    	height: 700, 
                    	url: 'canalCloseFlow/canalCloseReasonCWSH.jsp?inId='+row.inId+'&closeId='+row.closeId+'&nextStepVal='+row.nextStepVal+'&canalName='+canalName+'&money='+row.money,                    			
                    	 showMax: true, 
                    	 isResize: true, 
