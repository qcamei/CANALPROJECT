...
<title>渠道关闭信息首列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'070501','070502'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	        ...
	        grid = manager = $("#maingrid4").ligerGrid({
	        	 columns: [
	    					...
	    					rownumbers:false, 
	    					checkbox:false,
	    		            url: 'canalCloseQueryDLSGLAction?type=dlsgl',
	...
	if(ints2!=4){
		...
		if('通过'==row.auditState){
            var m= $.ligerDialog.open({ 
                    	...
                    	width: 400, 
                    	height: 700, 
                    	url: 'canalCloseFlow/canalCloseReasonDLSGL.jsp?inId='+row.inId+'&closeId='+row.closeId+'&nextStepVal='+row.nextStepVal,                    			
                    	 showMax: true, 
                    	 isResize: true, 
