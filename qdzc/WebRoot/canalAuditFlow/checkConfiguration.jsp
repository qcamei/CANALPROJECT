...
<title>配置稽核人员 列表页面</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050801','050802','050803','050804'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
               ...
               { display: '是否驳回', name: 'isBack',width: 50, minWidth: 60,
				...
				{ display: '稽核人员', name: 'checkUser', width: 60, minWidth: 60},
                ...
                url: 'checkConfigurationQueryAction',
                dataAction: 'server',
                pageSize:30,
                where : f_getWhere(),
                rownumbers:false, 
          ...
          function f_excelOut() {
        	    ...
        	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes)
	          		window.open('canalCheckConfigurationInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="业务稽核系统配置"/></td>
...
function f_add(){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=checkConfiguration',
	  showMax: true, 
	  isResize: true
  }); 
  m.max();
