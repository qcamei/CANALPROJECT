...
<title>财务审核环节</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'051401','051402','051403','051404'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
        ...
        grid = manager = $("#maingrid4").ligerGrid({
				...
				{ display: '是否收款', name: 'isPay', width: 50, minWidth: 60 },
				{ display: '本环节状态', name: 'auditState', width: 70, minWidth: 60 },
          ...
          function f_excelOut() {
      	    ...
      	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes)
	          		window.open('canalFinanceInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="财务审核"/></td>
...
function f_add(){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 500, 
	  width: 700, 
	 url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=finance',		
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
