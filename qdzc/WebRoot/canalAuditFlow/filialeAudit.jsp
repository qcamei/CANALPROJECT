...
<title>分公司审核列表页面</title>
    ...
    var manager=null;
        $(function() {
        	...
        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050101','050102','050103','050104'"></pageADMOperAuth.tld:pageADMOperAuth>
      ...
      /*  function get_curDay() {
        ...
        grid = manager = $("#maingrid4").ligerGrid({
               ...
               { display: '是否驳回', name: 'isBack',width: 50, minWidth: 60,
				...
				{ display: '代理商ID', name: 'agentId', width: 70, minWidth: 60 },
          ...
          function f_excelOut() {
      	    ...
      	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes) 
	          		window.open('canalFilialeAuditInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
...
function f_add(){
	...
	if(nextStepVal != "分公司审核" || isBack == "驳回"){
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=filialeAudit',
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
