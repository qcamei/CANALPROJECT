...
<title>第八步 资金稽核编码列表页面</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050601','050602','050603','050604'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
    ...
    /* function get_curDay() {
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
				...
				{ display: '代理商编码', name: 'agentId', width: 80, minWidth: 60,hide:1 },
				{ display: '归属代理商', name: 'agentName', width: 80, minWidth: 60 },
              
				{ display: '稽核厅编码', name: 'checkNumber', width: 80, minWidth: 60 },
				{ display: '稽核厅名称', name: 'checkName', width: 80, minWidth: 60 },
				{ display: '系统登录账号', name: 'moneyLogin', width: 85, minWidth: 60 },
          ...
          function f_excelOut() {
      	    ...
      	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes)
	          		window.open('canalMoneyCheckInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="资金稽核系统配置"/></td>
...
function f_add(){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=moneyCheck',
	  showMax: true, 
	  isResize: true
  }); 
  m.max();
