...
<title>基础设施配置 列表页面</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'051001','051002','051003','051004'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
               ...
               { display: '是否驳回', name: 'isBack',width: 50, minWidth: 60,
				...
				{ display: '一键式', name: 'dx360Number', width: 60, minWidth: 60},
				{ display: '受理设备类型', name: 'terminalType', width: 100, minWidth: 60},
				{ display: 'CPU', name: 'cpu', width: 60, minWidth: 60},
				{ display: '核数', name: 'kernal', width: 60, minWidth: 60},
				{ display: '主频', name: 'frequency', width: 60, minWidth: 60},
				{ display: '内存', name: 'memory', width: 60, minWidth: 60},
				{ display: '硬盘', name: 'disk', width: 60, minWidth: 60},
				{ display: '打印机型号', name: 'printNumber', width: 100, minWidth: 60},
				{ display: '是否无纸化', name: 'isNoPaper', width: 100, minWidth: 60},
                ...
                url: 'deviceConfigurationQueryAction',
                dataAction: 'server',
                pageSize:30,
                where : f_getWhere(),
                rownumbers:false, 
          ...
          function f_excelOut() {
      	    ...
      	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes)
	          		  window.open('canalDeviceConfigurationInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td>
						<input	name="canalId"  id="canalId"  type="text"/>
					</td>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="设备基础配置"/></td>
...
function f_add(){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	   url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=deviceConfiguration',	 
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
