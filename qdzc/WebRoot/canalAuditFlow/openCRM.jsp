...
<title>第六步 开CRM工号列表页面</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050401','050402','050403','050404'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
    ...
    /* function get_curDay() {
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
				...
				{ display: '工号权限', name: 'privilege', width: 80, minWidth: 60 ,hide:1},
          ...
          function f_excelOut() {
	        	...
	        	var canalId= encodeURI(encodeURI($("#canalId").val()));
	           	...
	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	           	  if(yes)
	           		  window.open('canalOpenCRMInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
...
function f_add(){
	...
	privilege = encodeURIComponent(encodeURIComponent(privilege));
	var m = $.ligerDialog.open({ 
	  title: '审核操作',
	  height: 400, 
	  width: 700, 
	   url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&privilege='+privilege+'&process=openCRM',
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
