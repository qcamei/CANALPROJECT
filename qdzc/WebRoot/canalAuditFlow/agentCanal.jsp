...
<html>
<head>
<title>第四五步 代理商系统渠道信息确认信息列表</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050301','050302','050303','050304'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
   ...
   /*  function get_curDay() {
	        	...
	        	var canalId= encodeURI(encodeURI($("#canalId").val()));
	             var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	
	           	  if(yes)
	    	            	   window.open('"canalAgentInfoDownExcel"?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
    ...
    <style type="text/css">
body{ font-size:12px;background:#f9fbfc}
.blackbold_b { /* 标题样式 */
	line-height: 22px;
	width: 150px;
	height: 22px;
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td>
						<input	name="inId"  id="inId"  type="text"/>
					</td>
					...
					<td>
						<input	name="canalName"    id="canalName"  type="text"/>
					</td>		
					<td align="right" class="l-table-edit-td">下一状态:</td>
					<td>
						<input	name="nextStepVal"    id="nextStepVal"  type="text" value="代理商管理系统"/>
					</td>
				...
				<tr>
						...
						<td>
							<input type="button"	value="查询全部" id="Button2" class="l-button" style="width:80px" onclick="f_query_all()" />
...
function f_add(){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 500, 
	  width: 700, 
	 // url: 'canalAuditFlow/agentCanalAdd.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&agentId='+agentId+'&agentName='+agentName,
		url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&agentName='+agentName+'&process=agentCanal',
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
