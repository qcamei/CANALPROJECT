...
<title>专线受理 列表页面</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'050901','050902','050903','050904'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
				...
				{ display: '开通时间', name: 'openTime', width: 60, minWidth: 60},			
          ...
          function f_excelOut() {
        	    ...
        	    var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	          	  if(yes)
	          		  window.open('canalConfigLineInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="专线受理确认"/></td>
...
function f_add(){
	...
	if(currentStep != "8" || parallelStep != "yes"){//这个地方yes就表示专线受理
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	   url: 'canalManage/detail.jsp?inId='+inId+'&type=add'+'&canalName='+canalName+'&canalId='+canalId+'&process=configLine',	 
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
