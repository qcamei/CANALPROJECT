...
<title>翼支付</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'051501','051502','051503','051504'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
        	 ...
        	 columns: [
                       ...
                       { display: '并行状态', name: 'yzfParallelStep',width: 50, minWidth: 60,render:function(item){
                       	var step = "";
                        if(item.yzfParallelStep == "yes"){
                        	step = "翼支付";
                        }
                        else if(item.yzfParallelStep == "no"){
                        	step = "无";
                        }
        				return step;
       				 } },
                     ...
                     { display: '翼支付账号', name: 'inCollectNumber', width: 60, minWidth: 60},
       				...
       				{ display: '户名', name: 'hm', width: 80, minWidth: 60},
       				{ display: '开户银行', name: 'khyh', width: 80, minWidth: 60},
       				{ display: '卡号', name: 'kh', width: 80, minWidth: 60},
       				{ display: '身份证号', name: 'sfzh', width: 80, minWidth: 60},
          ...
          function f_excelOut() {
	        	...
	        	var canalId= encodeURI(encodeURI($("#canalId").val()));
	          	...
	          	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	
	          	            if(yes)
	    	            	   window.open('canalBestoayInfoDownExcel?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="资金归集账号配置"/></td>
...
function f_add(){
	...
	if(currentStep <= "3" || currentStep == "11" || parallelStep=="yes"){
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalManage/detail.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&hm='+hm+'&khyh='+khyh+'&kh='+kh+'&sfzh='+sfzh+'&process=bestoay',
	  showMax: true, 
	  isResize: true
  }); 
  m.max();
