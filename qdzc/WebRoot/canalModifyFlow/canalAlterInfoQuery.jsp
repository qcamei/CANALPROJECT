...
<title>渠道变更信息首列表</title>
	...
	<script>
	 ...
	 var manager=null;     
	        $(function() {        
	        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'060101','060102','060103','060104','060105'"></pageADMOperAuth.tld:pageADMOperAuth>       	
	            var v = $("form").validate({
	                //调试状态，不会提交数据的
	                debug: true,
	       ...
	       function f_excelOut() {
	 	        	...
	 	        	var canalId= encodeURI(encodeURI($("#canalId").val()));
	 	             var canalUserPhone = encodeURI(encodeURI($("#canalUserPhone").val()));	                	
	 	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	 	    	            	   window.open('"canalAlterInfoDownExcel"?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&canalUserPhone='+canalUserPhone);
	 	           	   			window.close();
	 	    	            });
...
function f_modify(){//对应变更申请
		var row = manager.getSelectedRow();
		var m = $.ligerDialog.open({ 
    	  title: '渠道变更',
    	  height: 400, 
    	  width: 700, 
    	  url: 'canalModifyFlow/alterApply.jsp', 
    	  showMax: true, 
    	  isResize: true
      }); 
      m.max();
}
function modify(){   //对应变更申请工单在被驳回后可以在这里修改
	var row = manager.getSelectedRow();
   	if (!row) { 
   ...
   if(isBack != "驳回"){
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalModifyFlow/canalInfoXiugai_modify.jsp?canalId='+row.canalId, 
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
}
function f_detail(){//显示详细信息（只有必填和选填）
	var row = manager.getSelectedRow();
   	if (!row) { 
   		$.ligerDialog.error('请选择需要操作的行！');
   		return ; 
