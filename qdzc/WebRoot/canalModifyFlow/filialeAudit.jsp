...
<title>分公司审核列表页面（渠道变更流程）</title>
    ...
    var manager=null;
        $(function() {
			//配置三级页面的操作权限，配置toolBar
        	<pageADMOperAuth.tld:pageADMOperAuth menuId="'060201','060202','060203','060204','060205'"></pageADMOperAuth.tld:pageADMOperAuth>
       ...
       function f_initGrid() {
        ...
        grid = manager = $("#maingrid4").ligerGrid({
                ...
                { display: '下一状态ID', name: 'currentStep',width: 80, minWidth: 60, hide:1 },
                ...
                { display: '原因', name: 'reason', width: 70, minWidth: 60 },
				...
				{ display: '代理商编码', name: 'agentId', width: 70, minWidth: 60 },
				{ display: '归属代理商', name: 'agentName', width: 80, minWidth: 60 },
				{ display: '客户经理', name: 'canalUserName', width: 80, minWidth: 60 },
	              
				{ display: '审核部门', name: 'deptName',  width: 60, minWidth: 60 },
                { display: '审核操作人', name: 'operUser', width: 60, minWidth: 60},
                { display: '审核时间', name: 'operTime', width: 100, minWidth: 60 },                
                { display: '审核状态', name: 'auditState', width: 60, minWidth: 60},
                { display: '审核备注', name: 'auditRemark', width: 50, minWidth: 60 }
                ], 
                url: 'filialeAuditQuery',
                dataAction: 'server',
                pageSize:30,
                where : f_getWhere(),
                rownumbers:false, 
        ...
        function f_query(){        
          ...
          function f_query_all(){        
       		...
       		var nextStepVal = encodeURI($("#nextStepVal").val());
          	manager.setOptions(
                { parms: [{name:'nextStepVal',value:nextStepVal},
                               {name:'txtBeginDate',value:txtBeginDate},
                               {name:'txtEndDate',value:txtEndDate}] 
                }
           );
           manager.loadData(true);	             
          ...
          function f_excelOut() {
	            ...
	            var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	           	  if(yes)
	           		  window.open('"canalAlterFilialeAuditInfoDownExcel"?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
...
function f_modify(){
	...
	var inId = row.inId;
	var nextStepVal = row.nextStepVal;
	var isBack = row.isBack;
	if(isBack == "驳回"){
		alert("该状态下不能做此操作！");
		return;
	}
	if(nextStepVal != "分公司审核"){
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  	...
	  	height: 400, 
	  	width: 700, 
	  	url: 'canalModifyFlow/detailModify.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=filialeAudit', 
	   	showMax: true, 
	  	isResize: true
   }); 
   m.max();
...
function getDetails(){   //获得渠道全视图
	var row = manager.getSelectedRow();
	if (!row) { $.ligerDialog.error('请选择需要详细信息的工单！'); return; }
     var m = $.ligerDialog.open({ 
    	  	title: '详细信息',
    	 	height: 500, 
    	  	width: 800, 
