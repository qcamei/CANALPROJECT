...
<title>第三步 分管部门审核列表页面(变更)</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'060301','060302','060303','060304','060305'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
    ...
    /* function get_curDay() {
     ...
     function get_lDay() {   //获取本月1号
        ...
        grid = manager = $("#maingrid4").ligerGrid({
                ...
                { display: '操作时间', name: 'operTime', width: 120, minWidth: 60 },                
          ...
          function f_excelOut() {
	            ...
	            var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	    	            
	           	  if(yes)
	           		  window.open('"canalAlterSubDeptInfoDownExcel"?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="分管部门审核"/></td>
...
function f_modify(){
	...
	if(nextStepVal != "分管部门审核" || isBack == "驳回"){
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 400, 
	  width: 700, 
	  url: 'canalModifyFlow/detailModify.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&process=subDeptAudit', 
	   showMax: true, 
	   isResize: true
  }); 
  m.max();
