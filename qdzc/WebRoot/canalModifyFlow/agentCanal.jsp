...
<html>
<head>
<title>第四五步 代理商系统渠道信息确认信息列表（变更流程）</title>
    ...
    var manager=null;
    $(function() {        
    	<pageADMOperAuth.tld:pageADMOperAuth menuId="'060401','060402','060403','060404','060405'"></pageADMOperAuth.tld:pageADMOperAuth>       	
        var v = $("form").validate({
            //调试状态，不会提交数据的
            debug: true,
          ...
          function f_excelOut() {
	            ...
	            var nextStepVal = encodeURI(encodeURI($("#nextStepVal").val()));	                	
	           	$.ligerDialog.confirm('确认要导出渠道信息吗？', function (yes){	  
	           	  if(yes)
	    	            	   window.open('"canalAlterAgentInfoDownExcel"?txtBeginDate='+txtBeginDate+'&txtEndDate='+txtEndDate+'&inId='+inId+'&areaName='+areaName+'&canalName='+canalName+'&canalId='+canalId+'&nextStepVal='+nextStepVal);
	           	   			window.close();
	    	            });
		...
		<form name="form1" method="post" action="" id="form1">
			...
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				...
				<tr>
					...
					<td><input	name="nextStepVal"    id="nextStepVal"  type="text" value="代理商管理系统"/></td>
...
function f_modify(){
	...
	if(nextStepVal != "代理商管理系统" || isBack == "驳回"){
		alert("该状态下不能做此操作！");
		return;
	}
	...
	var m = $.ligerDialog.open({ 
	  ...
	  height: 500, 
	  width: 700, 
	  url: 'canalModifyFlow/detailModify.jsp?inId='+inId+'&canalName='+canalName+'&canalId='+canalId+'&agentId='+agentId+'&agentName='+agentName+'&process=agentCanal', 
	   showMax: true, 
	   isResize: true
  }); 
	m.max();
