$(function(){
	...
	$("#canalXingzhi").ligerComboBox({url :'selectItemQueryAction?type=canal_xingzhi', isMultiSelect: false});
	...
	$("#taskType").ligerComboBox({
		...
		onSelected:function(result){
			if(result == '1'){
				...
				$("#privilege").val("普通营业员权限，不开销帐权限");
			}else if(result == '2'){
				$("#guaranteeAmount").val("10000");
				$("#privilege").val("普通营业员权限，开销帐权限");
			}else if(result == '3'){
...
function submit_Result(result) { //回传函数实体，参数为XMLhttpRequest.responseText   
	...
	//验证电话号码和身份证号
	function checklegal(id){   
	  var ab=/^(17[0-9]|13[0-9]|15[0-9]|18[0-9])\d{8}$/;
	  //var ab=/^(([0+]d{2,3}-)?(0d{2,3})-)?(d{7,8})(-(d{3,}))?$/;
	  ...
	  if(legalIdCard != null && legalIdCard !=""){
			if (cd.test(legalIdCard) == false) {
				$.ligerDialog.error("身份证号不合法!");
				$("#legalIdCard").val("");
				return false;
			}
	  }
	  if(sfzh != null && sfzh !=""){
			if (cd.test(sfzh) == false) {
				$.ligerDialog.error("身份证号不合法!");
				$("#sfzh").val("");
				return false;
			}
