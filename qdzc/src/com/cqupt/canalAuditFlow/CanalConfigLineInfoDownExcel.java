...
public class CanalConfigLineInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "并行状态", "是否驳回",
					...
					"管理属性", "分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "开通时间",
			...
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"canalParentName", "agentName", "openTime", "deptName",
					"operUser", "checkTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*,date_format(a.open_time,'%Y-%m-%d') open_time,a.oper_user,a.dept_name,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') check_time from "
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process9_config_line a on t.in_id=a.in_id where 1=1";
			...
			if (!nextStepVal.equals("")) {
				if (nextStepVal.equals("专线受理确认")) {
					sql += " and t.current_step = 8 and t.parallel_step = 'yes'";
				} else {
