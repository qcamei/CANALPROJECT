...
public class CanalBestoayInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			if (nextStepVal.equals("")) {
				nextStepVal = "资金归集账号配置";
			} else if (nextStepVal.equals("全部")) {
				nextStepVal = "";
			}

			String[] title = { "序号", "工单号", "当前状态", "下一状态", "并行状态", "是否驳回",
					"驳回原因", "渠道编码", "工号", "翼支付账号", "渠道名称", "区域", "状态", "城乡",
					"管理属性", "分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "户名",
					"开户银行", "卡号", "身份证号", "操作部门", "操作人", "操作时间", "备注" };// excel工作表的标题
			String[] keys = { "rownum", "inId", "currentStepVal",
					"nextStepVal", "yzfParallelStep", "isBack", "reason",
					"canalId", "crmNumber", "inCollectNumber", "canalName",
					"areaName", "canalState", "canalForm", "canalProperty",
					"canalType", "canalUserName", "broadbandAccount",
					"canalParentName", "agentName", "hm", "khyh", "kh", "sfzh",
					"deptName", "operUser", "checkTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,"
					+ "(select t.*,"
					+ "a.oper_user,a.dept_name,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') check_time from "
					+ " (select t.*,a.step_val next_step_val,a.pre_step_val current_step_val from"
					+ " ( SELECT a.in_id,a.in_collect_number,a.canal_id,a.crm_number,a.canal_name,a.area_name,a.canal_state,a.canal_form,"
					+ "a.canal_property,a.canal_type,a.canal_parent_name,a.agent_name,a.canal_user_name,"
					+ "a.canal_user_phone,a.broadband_account,a.urban_idetity,a.hm,a.khyh,a.kh,a.sfzh,"
					+ "date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') oper_time,b.current_step,b.reason,"
					+ "case when b.yzf_parallel_step=14 then 'yes' when b.yzf_parallel_step=0 then 'no' else b.yzf_parallel_step end yzf_parallel_step,b.is_back"
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process14_bestoay a on t.in_id=a.in_id where 1=1";
			...
			if (!nextStepVal.equals("")) {
				if (nextStepVal.equals("资金归集账号配置")) {
					sql += " and (t.current_step BETWEEN 4 and  10 or t.current_step = 13 ) and t.yzf_parallel_step != 'yes'";
				} else {
