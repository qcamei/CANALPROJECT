...
public class CanalDeviceConfigurationInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "驳回原因",
					...
					"分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "一键式", "受理设备类型",
					"CPU", "核数", "主频", "内存", "硬盘", "打印机型号", "是否无纸化", "操作部门",
					"操作人", "操作时间", "备注" };// excel工作表的标题
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"dx360Number", "terminalType", "cpu", "kernal",
					"frequency", "memory", "disk", "printNumber", "isNoPaper",
					"deptName", "operUser", "checkTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,"
					+

					"(select t.*,a.dx360_number,a.terminal_type,a.terminal_number,a.cpu,a.kernal,a.frequency,a.memory,a.disk,a.print_number,a.is_no_paper,"
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process10_device_configuration a on t.in_id=a.in_id where 1=1";
