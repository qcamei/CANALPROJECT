...
public class CanalCheckConfigurationInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "驳回原因",
					...
					"分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "稽核人员", "操作部门",
			...
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"checkUser", "deptName", "operUser", "checkTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*,a.check_user,a.oper_user,a.dept_name,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') check_time from "
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process8_check_configuration a on t.in_id=a.in_id where 1=1";
