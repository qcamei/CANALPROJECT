...
public class CanalLineConfig12InfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "并行状态", "是否驳回",
					...
					"管理属性", "分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "专网账号",
					"专网密码", "外网账号", "外网优惠", "操作部门", "操作人", "操作时间", "备注" };// excel工作表的标题
			String[] keys = { "rownum", "inId", "currentStepVal",
					"nextStepVal", "parallelStep", "isBack", "reason",
					...
					"broadbandPassword", "wwwAccount", "wwwPrivilegeInfo",
					"deptName", "operUser", "checkTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,"
					+ "(select t.*,a.broadband_account,a.broadband_password,a.www_account,a.www_privilege_info,"
					...
					+ "a.canal_user_phone,a.in_collect_number,a.urban_idetity,"
					+ "date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') oper_time,b.current_step,b.reason,"
					// 因为值为中文时乱码，所以用yes,no
					+ "case when b.parallel_step=12 then 'yes' when b.parallel_step=0 then 'no' else b.parallel_step end parallel_step,b.is_back"
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process12_config_line a on t.in_id=a.in_id where 1=1";
			...
			if (!nextStepVal.equals("")) {
				if (nextStepVal.equals("专线受理")) {
					sql += " and (t.current_step BETWEEN 4 and  8 or t.current_step = 13 ) and t.parallel_step != 'yes'";
				} else {
