...
public class CanalFinanceInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "驳回原因",
					...
					"分类名称", "客户经理", "VPDN", "上级渠道", "代理商编码", "归属代理商", "是否收款",
					"本环节状态", "操作部门", "操作人", "操作时间", "备注" };// excel工作表的标题
			...
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"agentName", "isPay", "auditState", "deptName", "operUser",
					"operTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*, a.is_pay,a.audit_state,a.dept_name,a.oper_user,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') check_time from "
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process13_finance a on t.in_id=a.in_id where 1=1";
