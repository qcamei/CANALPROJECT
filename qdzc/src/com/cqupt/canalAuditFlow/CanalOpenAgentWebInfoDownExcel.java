...
public class CanalOpenAgentWebInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "驳回原因",
					...
					"分类名称", "客户经理", "VPDN", "上级渠道", "归属代理商", "代理商门户网站工号",
					"稽核厅名称", "操作部门", "操作人", "操作时间", "备注" };// excel工作表的标题
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"agentJobNumber", "employeeName", "deptName", "operUser",
					"operTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*,a.agent_job_number,a.employee_name,a.oper_user,a.dept_name,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') check_time from "
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process6_open_agentNumber a on t.in_id=a.in_id where 1=1";
