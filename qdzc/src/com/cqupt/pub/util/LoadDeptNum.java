...
public class LoadDeptNum extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			sql = "select dept_id, contact_num from qdzc.sys_dept";
			...
			for(int i = 0; i < list.size(); i ++) {
				...
				if(map.get("contactNum").toString().equals("")){
					logger.info("部门编号为" + map.get("deptId") + "的部门联系电话为空！");
				} else {
					context.setAttribute(map.get("deptId") + "TelNumber", map.get("contactNum"));
				}
				
			//	context.setAttribute(map.get("deptId") + "TimeLimit", map.get("timeLimit"));
				//System.out.println(map.get("deptId") + "TelNumber:" + map.get("timeLimit"));
