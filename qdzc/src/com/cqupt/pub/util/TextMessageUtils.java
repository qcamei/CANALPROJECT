...
public class TextMessageUtils {
	...
	public static boolean alertSourceDepartment(String caseNum) {
		...
		try {
			...
			sql = "select a.dept_name, a.dept_id, b.current_dept_name from qdzc.case_accept as a inner join qdzc.case_status as b on a.case_num=b.case_num where a.case_num='" + caseNum + "'";
	...
	public static boolean alertRepairDepartment(String caseNum) {
		...
		try {
			session = DataStormSession.getInstance();
			sql = "select a.launch_dept_name, a.receive_dept_id, a.receive_dept_name from qdzc.case_launch as a where a.launch_status='待接收' and a.case_num='" + caseNum + "'";
			list = session.findSql(sql);
	...
	public static boolean alertUser(String caseNum) {
		...
		try {
			session = DataStormSession.getInstance();
			sql = "select a.client_name, a.contact_number, a.dept_name, b.case_status, date_format(a.accept_time,'%Y年%m月%d日') as accept_time from qdzc.case_accept as a inner join qdzc.case_status as b on a.case_num=b.case_num where a.case_num='" + caseNum + "'";
			list = session.findSql(sql);
			if(list.size() == 0) {
