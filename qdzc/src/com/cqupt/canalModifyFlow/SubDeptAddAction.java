	...
	private String insertIntoDatabase() {
		...
		try {
			...
			if (resultStr.equals("success")) {
				...
				// 根据是否变更保证金判断流程是到分管部门审核结束
				sql = "select is_alter_money from qdzc.canal_infomation_his where in_id='"
						+ inId + "'";
				Map resultMap2 = session.findSql(sql).get(0);
				String isAlterMoney = (String) resultMap2.get("isAlterMoney");
				if (isAlterMoney.equals("否")) {
					sql = "SELECT * FROM step_info_modify WHERE step_no='7'";
				...
				if (auditState.equals("通过")) {
					sql = "update qdzc.canal_step_state_his set current_step = '"
							+ currentStep
							+ "',department_audit_time=sysdate(),"
							+ "department_audit_name='"
							+ currentStepVal
							+ "',department_audit_state='"
							+ auditState
							+ "',is_back='"
							+ isBack
							+ "' where in_id='"
					...
					session.update(sql);
					// 4.操作入环节“配置稽核人员”
					sql = "insert into qdzc.process3_department_audit_his (in_id,canal_id,canal_name,oper_user,dept_id,dept_name,oper_time,audit_state,remark) values ("
							+ "'"
							+ inId
							+ "','"
							+ canalId
					...
					sql = "update qdzc.canal_step_state_his set branch_audit_time=sysdate(),branch_audit_state='"
							+ auditState
							+ "',is_back='"
							+ isBack
							+ "',reason='"
				...
				// sql =
				// "select canal_user_name from qdzc.canal_infomation where in_id = '"
				// + inId + "'";
