...
public class FilialeAuditAddAction extends ActionSupport {
	...
	private String insertIntoDatabase() {
		...
		try {
			...
			if (resultStr.equals("success")) {
				// 1.判断环节“配置稽核人员”
				sql = "select * from qdzc.canal_step_state_his where  in_id = '"
						+ inId + "'";
				logger.info(sql);
			...
			if (resultStr.equals("success")) {
				...
				if (auditState.equals("通过")) {
					...
					session.update(sql);
					// 4.操作入环节“分公司审核”
					sql = "insert into qdzc.process2_branch_audit_his (in_id,canal_id,canal_name,audit_state,oper_user,dept_id,dept_name,oper_time,audit_remark) values ("
							+ "'"
							+ inId
							+ "','"
							+ canalId
				...
				// sql =
				// "select canal_user_name from qdzc.canal_infomation where canal_id = '"
				// + canalId + "'";
				// List list2 = session.findSql(sql);
				// Map map = (Map) list2.get(0);
				// String user = map.get("canalUserName").toString();
				// // 分公司审核-->分管部门审核
				// MailRemindUtil.mailFGBM(user, canalName);
