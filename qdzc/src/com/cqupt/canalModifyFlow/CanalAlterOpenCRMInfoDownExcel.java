...
public class CanalAlterOpenCRMInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "操作个数",
					"渠道编码", "渠道名称", "区域", "状态", "城乡", "管理属性", "分类名称", "客户经理",
					"VPDN", "上级渠道", "归属代理商", "CRM工号", "绑定手机号码", "操作部门", "操作人",
					"操作时间", "备注" };// excel工作表的标题
			String[] keys = { "rownum", "inId", "currentStepVal",
					"nextStepVal", "isBack", "number", "canalId", "canalName",
					"areaName", "canalState", "canalForm", "canalProperty",
					"canalType", "canalUserName", "broadbandAccount",
					"canalParentName", "agentName", "crmNumber", "telephone",
			...
			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,"
					...
					+ "a.canal_user_phone,a.broadband_account,a.number,a.urban_idetity,"
