...
public class CanalAlterFinanceInfoDownExcel extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			String[] title = { "序号", "工单号", "当前状态", "下一状态", "是否驳回", "原因",
					...
					"VPDN", "上级渠道", "代理商编码", "归属代理商", "是否收款", "本环节状态", "操作部门",
			...
			String[] keys = { "rownum", "inId", "currentStepVal",
					...
					"canalParentName", "agentId", "agentName", "isPay",
					"auditState", "deptName", "operUser", "operTime", "remark" };

			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*,a.audit_state,a.oper_user,a.dept_name,a.is_pay,a.remark,date_format(a.oper_time,'%Y-%m-%d %H:%i:%s') oper_time from "
			...
			sql += " ORDER BY a.in_id DESC) as t  left join"
					...
					+ " left join qdzc.process13_finance_his a on t.in_id=a.in_id where 1=1";
