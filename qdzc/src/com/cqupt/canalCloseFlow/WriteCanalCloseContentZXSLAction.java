...
public class WriteCanalCloseContentZXSLAction extends ActionSupport {
	// 专线受理审核函数
			...
			if (resultStr.equals("success")) {
				...
				if ("通过".equals(auditState)) {
					sql = "insert into qdzc.canal_infomation_close(close_id,user_name,dept_name,dept_id,audit_state,canal_id,canal_name,current_step_val,current_step,next_step_val,oper_time,in_id,refuse_reason)values('"
							...
							+ "','" + refuseReason + "')";
					session.add(sql);
					logger.info("添加流程表：" + sql);
					// 添加流程状态表
					sql = "update  qdzc.canal_step_state_close set oper_user8="
							+ "'" + userName + "',dept_id8='" + deptId
							+ "',dept_name8='" + deptName + "',"
							+ " config_line_time='" + operTime + "', "
							+ "config_line_name='" + currentStepVal
							+ "',config_line_state='" + auditState
