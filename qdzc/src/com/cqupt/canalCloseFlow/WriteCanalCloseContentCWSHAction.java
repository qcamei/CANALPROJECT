...
public class WriteCanalCloseContentCWSHAction extends ActionSupport {
	// 分管部门审核函数
	...
	public String execute() {
		...
		try {
			...
			String money = java.net.URLDecoder.decode(
					request.getParameter("money"), "UTF-8");

			String bmoney = java.net.URLDecoder.decode(
					request.getParameter("guaranteeAmount"), "UTF-8");
			String returnMoney = java.net.URLDecoder.decode(
					request.getParameter("returnMoney"), "UTF-8");
			...
			// 直接输入响应的内容
			out.print(getList(inId, closeId, refuseReason, auditState, money,
					bmoney, returnMoney, deptName, deptId, userName));
			out.flush();
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public String getList(String inId, String closeId, String refuseReason,
			String auditState, String money, String bmoney, String returnMoney,
			String deptName, String deptId, String userName) {
		...
		try {
			float b;
			// 为了处理数据中保证金金额为null
			if (("null").equals(bmoney)) {
				b = 0;
			} else {
				b = Float.parseFloat(bmoney);
			}
			...
			if (resultStr.equals("success")) {
				...
				if ("通过".equals(auditState)) {
					sql = "insert into qdzc.canal_infomation_close(close_id,user_name,dept_name,dept_id,audit_state,canal_id,canal_name,current_step_val,current_step,next_step_val,oper_time,in_id,refuse_reason,money,bmoney,return_money)values('"
							+ closeId
							+ "','"
							+ userName
							+ "','"
					...
					// 添加流程状态表
					sql = "update  qdzc.canal_step_state_close set oper_user11="
							+ "'"
							+ userName
							+ "',dept_id11='"
							+ deptId
					...
					sql = "update qdzc.canal_infomation  set canal_state='关闭' where in_id='"
							+ inId + "'";
					session.update(sql);
