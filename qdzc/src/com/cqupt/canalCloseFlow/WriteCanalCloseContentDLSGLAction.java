...
public class WriteCanalCloseContentDLSGLAction extends ActionSupport {
	// 分管部门审核函数
		...
		try {
			...
			if (resultStr.equals("success")) {
				...
				if ("通过".equals(auditState)) {
					...
					// 添加流程状态表
					sql = "update  qdzc.canal_step_state_close set oper_user4="
							+ "'" + userName + "',dept_id4='" + deptId
							+ "',dept_name4='" + deptName + "',"
							+ " agent_canal_time='" + operTime + "', "
							+ "agent_canal_name='" + currentStepVal
							+ "',agent_canal_state='" + auditState
					...
					// 分公司审核-->分管部门审核
					MailRemindUtil.mailQDGS(user, canalName);
