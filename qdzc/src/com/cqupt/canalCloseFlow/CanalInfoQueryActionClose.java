...
public class CanalInfoQueryActionClose extends ActionSupport {
	//
	...
	private String getDate() {
		...
		try {
			...
			// 王志迁修改
			// canalState = DecodeUtils.decodeRequestString(request
			// .getParameter("canalState"));
			inId = DecodeUtils
			...
			sql = "SELECT 	@rownum :=@rownum + 1 AS rownum,t.* from (select @rownum:=0) r,(select t.*,a.step_val next_step_val,a.pre_step_val current_step_val from"
					...
					+ " where a.dept_id in ("
					+ dataAuthId
					+ ")  and a.canal_state='正常' and b.current_step='11'  ";
