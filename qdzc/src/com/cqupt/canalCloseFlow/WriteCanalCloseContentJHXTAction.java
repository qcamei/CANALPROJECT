...
public class WriteCanalCloseContentJHXTAction extends ActionSupport {
	// 分管部门审核函数
	...
	public String getList(String inId, String closeId, String refuseReason,
			String auditState, String money, String deptName, String deptId,
			String userName) {
		...
		try {
			...
			if (resultStr.equals("success")) {
				...
				if ("通过".equals(auditState)) {
					sql = "insert into qdzc.canal_infomation_close(close_id,user_name,dept_name,dept_id,audit_state,canal_id,canal_name,current_step_val,current_step,next_step_val,oper_time,in_id,refuse_reason,money)values('"
							+ closeId
							+ "','"
							+ userName
							+ "','"
					...
					// 添加流程状态表
					sql = "update  qdzc.canal_step_state_close set oper_user10="
							+ "'"
							+ userName
							+ "',dept_id10='"
							+ deptId
							...
							+ deptName
							+ "',"
							+ " check_configuration_time='"
							+ operTime
							+ "', "
							+ "check_configuration_name='"
							+ currentStepVal
							+ "',check_configuration_state='"
							+ auditState
							+ "',current_step='"
							+ currentStep
							+ "' where in_id='" + inId + "'";
